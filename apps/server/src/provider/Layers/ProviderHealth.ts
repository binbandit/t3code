...
> =>
  Effect.gen(function* () {
    ...
    const binaryLabel = binaryPath === "codex" ? "`codex`" : `\`${binaryPath}\``;

    // Probe 1: `codex --version` — is the CLI reachable?
    const versionProbe = yield* runCodexCommand(binaryPath, ["--version"]).pipe(
    ...
    if (Result.isFailure(versionProbe)) {
      ...
      return {
        ...
        message: isCommandMissingCause(error)
          ? `Codex CLI (${binaryLabel}) is not installed or not on PATH.`
    ...
    const authProbe = yield* runCodexCommand(binaryPath, ["login", "status"]).pipe(
      Effect.timeoutOption(DEFAULT_TIMEOUT_MS),
      Effect.result,
    );
...
export const ProviderHealthLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const runCheck = (binaryPath?: string) =>
      Effect.all([checkCodexProviderStatus(binaryPath), checkClaudeProviderStatus], {
        concurrency: "unbounded",
      }).pipe(
    ...
    const statusesFiber = yield* runCheck().pipe(Effect.forkScoped);
    ...
    return {
      ...
      getStatuses: Fiber.join(statusesFiber).pipe(Effect.andThen(Ref.get(statusesRef))),
